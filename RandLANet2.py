...
class Network:
    def __init__(self, dataset, config):#初始函数新建一个网络对象时都会调用它。
        ...
        with tf.variable_scope('optimizer'):
            ...
            self.tower_grads = []
            logits_list=tf.split(self.logits,2,0)
            labels_list=tf.split(self.labels,2,0)
            print('logits length is',tf.size(logits_list))
            for i in range(num_gpus):
                with tf.device(assign_to_device('/gpu:{}'.format(i), ps_device='/cpu:0')):
                   # self.loss=self.get_loss(logits_list[i],labels_list[i],self.class_weights)
                    self.grad=self.train_op.compute_gradients(self.loss)
                    self.tower_grads.append(self.grad)
            self.grads = average_gradients(self.tower_grads)
            self.train_op=self.train_op.apply_gradients(self.grads)
        ...
        c_proto.gpu_options.per_process_gpu_memory_fraction = 1  # 占用40%显存
    ...
    def get_loss(self, logits, labels, pre_cal_weights):
        ...
        weights = tf.reduce_sum(class_weights * one_hot_labels, axis=1)#训练的损失函数定义
